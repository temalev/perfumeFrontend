...
- [x] В hero добавлен реальный H1 «Оригинальная парфюмерия мировых брендов» с подзаголовком (доставка по Москве/Рязани/России) и CTA-кнопкой `NuxtLink → /products/list`. Старый закомментированный плейсхолдер убран.
- [x] Hero обёрнут в семантический `<section>` вместо `<div>`. Добавлен `.hero-overlay` (градиентное затемнение) — белый текст читается поверх светлой картинки. На мобиле градиент перевёрнут (снизу-вверх).
- [x] Картинка получила осмысленный alt `«Оригинальная парфюмерия мировых брендов в ПарфБюро»` (вместо `parfBuro background`).
- [x] `<h3>` секций «Специальные предложения» и «Хиты» понижены до `<h2>` — теперь иерархия валидна: один H1 + два H2.
- [x] Типографика hero: серифный/санс заголовок 56px (40/32 на меньших экранах), letter-spacing eyebrow, drop-shadow для контраста, CTA в виде пилюли с hover-инверсией цвета.
